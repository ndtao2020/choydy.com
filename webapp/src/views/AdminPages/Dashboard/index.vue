<template>
  <div class="dashboard-page">
          ...
          <Widget class="h-100 mb-0" title="Server Overview" close>
            <div class="d-flex align-items-center mb-sm">
              ...
              <div style="width: calc(100% - 150px)">
                <trend :data="getRandomData()" :gradient="['#FD5F00']" :height="40" stroke-width="4" smooth />
              </div>
            </div>
            <div class="d-flex align-items-center mb-sm">
              <p class="width-150"><small>54% / 31°С / 3.3 Ghz</small></p>
              <div style="width: calc(100% - 150px)">
                <trend :data="getRandomData()" :gradient="['#1A86D0']" :height="40" stroke-width="4" smooth />
              </div>
            </div>
            <div class="d-flex align-items-center">
              <p class="width-150"><small>57% / 21°С / 3.3 Ghz</small></p>
              <div style="width: calc(100% - 150px)">
                <trend :data="getRandomData()" :gradient="['#005792']" :height="40" stroke-width="4" smooth />
              </div>
            </div>
          </Widget>
        </div>
    ...
    <b-row>
      <b-col v-for="stat in mock.bigStat" :key="stat.id" xs="12" lg="6" xl="4">
        <BigStat :product="stat.product" :color="stat.color" :total="stat.total" :registrations="stat.registrations" :bounce="stat.bounce" />
      </b-col>
    </b-row>
    <b-row>
      <b-col xs="12">
        <Widget title="<h5>Support <span class='fw-semi-bold'>Requests</span></h5>" body-class="widget-table-overflow" custom-header>
            ...
            <table class="table table-striped table-lg mb-0 requests-table">
              ...
              <tbody>
                <tr v-for="row in mock.table" :key="row.id">
                  ...
                  <td>{{ row.city }}</td>
                  <td>
                    <b-badge :variant="row.status === 'Pending' ? 'success' : row.status === 'Declined' ? 'danger' : 'info'" pill>
                      {{ row.status }}
                    </b-badge>
                  </td>
                </tr>
