      ...
      <div class="iq-todo-page">
        <b-form class="position-relative">
          <b-form-group class="mb-0" label-for="search">
            <b-form-input v-model="search" class="todo-search" placeholder="Search" />
            <b-link href="#" class="search-link"><i class="ri-search-line" /></b-link>
          </b-form-group>
        </b-form>
        <div class="add-new-project mt-3 mb-3">
          <b-link v-b-modal.add_project href="#" class="d-block new-project" size="lg"><i class="ri-add-line mr-2" />Add Project</b-link>
          <b-modal id="add_project" centered title="Add Project">
            <p class="my-2">
              <b-form-input v-model="project.project_name" name="project_name" placeholder="Project Name" />
            </p>
            ...
            <ul :id="'todo-task' + index" class="sub-task mt-2 p-0" :class="`${item.id === selectedProject.id ? 'show' : ''}`">
              ...
              >
                <b-link :href="category.href"
                  ><i class="ri-checkbox-blank-circle-fill" :class="'text-' + category.color" /> {{ category.name }}</b-link
                >
              </li>
            </ul>
          </li>
